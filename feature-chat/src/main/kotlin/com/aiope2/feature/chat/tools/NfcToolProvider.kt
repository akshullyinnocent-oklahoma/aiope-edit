...
class NfcToolProvider(private val ctx: Context) {
  ...
  /** Enable/disable NFC */
  fun setEnabled(enable: Boolean): String = if (RootDetector.hasRootAccess()) {
    try {
      if (enable) {
  ...
  fun listTechnologies(): String = buildString {
  ...
  fun getMifareInfo(): String = buildString {
  ...
  fun buildSystemContext(): String = if (nfcAdapter != null) {
